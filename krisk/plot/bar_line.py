...
def set_bar_line_chart(chart, df, x, c, **kwargs):
    ...
    if kwargs['type'] == 'bar' and kwargs['trendline']:
        ...
        if c and kwargs['stacked']:
            ...
            raise AssertionError(
                'Trendline must either stacked category, or not category')
    ...
    if kwargs['type'] == 'hist' and kwargs['density']:
        ...
        chart.option['xAxis']['data'] = [0] + chart.option['xAxis'][
            'data'] + [0]
        ...
        if c and kwargs['stacked']:
            ...
            raise AssertionError(
                'Density must either stacked category, or not category')
...
def get_bar_line_data(df, x, c, y, **kwargs):
    ...
    if sort_on == 'index':
        ...
        if sort_on != 'values':
            val_deviation = data.describe().loc[sort_on] if isinstance(
                sort_on, str) else sort_on
